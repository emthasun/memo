class App {
  ...
  initializeScrollEvent() {
    ...
    this.animatedTexts = Array.from({ length: 29 }, (_, i) =>
  ...
  handleScroll() {
    ...
    if (scrollPosition > 32500) {
      window.location.href = "menuVisualOwn.html";
    }
  }
  ...
  trackMouseMovementReverse() {
    const colourWordsId = ["highlighted-text-20", "highlighted-text-24", "highlighted-text-11"];
    const elements = colourWordsId.map((id) => document.getElementById(id));
